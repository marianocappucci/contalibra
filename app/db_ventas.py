...
def _alicuota_de(venta_id: int) -> float | None:
    """Una venta que llegó de otro producto puede traer la suya (`ventas_origen_externo`,
    el caso de MedLibra con las prestaciones exentas). Import adentro: `db_integraciones`
    importa de `database`, que importa de acá."""
    from app.db_integraciones import get_alicuota_externa

    return get_alicuota_externa(venta_id)
