"""Ventas de Contalibra, sobre `sales`/`sale_items` de LibraCommerce —
P7 del plan de consolidación de la familia Libra (ver
...
- **LibraCommerce**: `sales` (encabezado) + `sale_items` (líneas, ahora
  ...
  (descuento por venta, vía `db_stock`).
...
del vertical" que ya usan VentaLibra y Gestiolibra/MedLibra. Las dos
familias de tablas viven en el MISMO archivo SQLite, así que un solo
`conn` cubre todo: si algo falla a mitad de camino, el rollback revierte
venta, líneas, stock, pagos, caja y turno juntos — igual que antes.

`libracore.db.ventas` sigue existiendo sin cambios para **Restolibra**,
que no se migra (su descuento de stock es receta-aware).
...
Contalibra, con `venta_id` = `sales.id`.
"""
import contextlib
import sqlite3
...
def create_venta(numero: str, fecha: str, items: list, subtotal: float,
    ...
    producto del catálogo) es 'service' sin `item_id` — la misma regla de
    negocio que ya enforcean el dominio de LibraCommerce y el `CHECK` de
    `sale_items`.
    """
    cm = contextlib.nullcontext(conn) if conn is not None else get_connection()
    with cm as c:
...
def crear_venta_directa(fecha: str, items: list, subtotal: float, descuento: float,
    ...
    turno activo — todo en una única transacción, cruzando LibraCommerce
    (venta/líneas/stock) y LibraCore (pagos/caja/turno).
...
def anular_venta(vid: int, usuario_id: int | None = None) -> None:
    ...
    pago a cuenta corriente, acredita la deuda del cliente. Todo en una única
    transacción; no-op si la venta ya estaba anulada, para no revertir dos
    veces si se reintenta la acción.

    La reversión del dinero es de LibraCore (`db.reversiones`, extraída el
    2026-07-28: el cuerpo de esta función era idéntico al de Restolibra). La
    reposición de stock se queda acá porque pasa por `add_movimiento_stock`,
    que resuelve las recetas y es propio del producto.
    """
    with get_connection() as conn:
        try:
            venta = conn.execute(
