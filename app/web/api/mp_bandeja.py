...
wiki/entities/contalibra.md, migracion a React). Reusa `db_mp.py`,
`mp_api.py` y `mp_facturacion.py` tal cual -- ver web/api/clientes.py
...
en produccion). El endpoint nuevo de aca los importa correctamente en
vez de replicar el bug.
"""
import datetime
import logging
import os
...
async def facturar_pago(mp_pago_id: int, payload: FacturarPayload):
    ...
    factura_id, numero, tipo_label, email_sent = await mp_facturacion.generar_factura_mp(
        ...
        payment_type=pago.get("payment_type") or "", payer_cuit=pago.get("payer_id_number") or "",
    )
...
async def facturar_movimiento(mov_id: int, payload: FacturarPayload):
    ...
    factura_id, numero, tipo_label, email_sent = await mp_facturacion.generar_factura_mp(
        ...
        payment_type=mov.get("tipo") or "", payer_cuit=mov.get("payer_id_number") or "",
    )
