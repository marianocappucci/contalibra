"""API JSON de Configuracion para la SPA (ver wiki/entities/contalibra.md,
...
en su totalidad (gateado en web/app.py con require_admin_json).

Los backups de la DB (`GET /config/backup-db[/{filename}]`, descargas de
archivo autenticadas por cookie) siguen sirviendose desde `web/routers/config.py` sin tocar -- la SPA los
linkea directo (misma cookie, mismo origen), no hace falta una version JSON de
un download.
...
# 🔴 `PUT /mp` se fue con `MercadoPagoPayload`: lo sirve
# `libracore.mp_config_router`. El del motor devuelve el token enmascarado, no
# pisa la credencial cuando el campo viene vacio, y tiene una puerta para
...
# superadmin (`admin.contalibra.com.ar`), que escribe el `config.json` de la
...
#
# El resumen de por que no se conservaron: los dos estaban rotos desde el corte
# a PostgreSQL y las dos formas de fallar escribian la contrasena de la base en
# el mensaje de error. El restore, ademas, exigia un archivo `SQLite format 3`
# -- o sea que rechazaba el `.dump` que el propio producto generaba-- y su
# "backup de seguridad previo" iba dentro de un `try/except: pass`, asi que
# podia no hacerse sin que nadie se enterara.
#
# Las rutas nuevas, del motor: `GET/POST /api/config/backups`,
...
# Router aparte, con el MISMO prefix. Mismo patron que `me_router` en
# web/api/usuarios.py, y por el mismo motivo: FastAPI evalua las dependencias
# del router antes que las de la ruta, asi que no alcanza con ponerle un
# guard distinto a cada endpoint.
#
# Se separa para que el token de servicio del backoffice de la suite abra
# UNICAMENTE el correo saliente. El resto de este router —ARCA, ticket, datos
# de empresa, MercadoPago— sigue admin-only: el backoffice no tiene por que
# poder tocar la configuracion fiscal de un cliente.
