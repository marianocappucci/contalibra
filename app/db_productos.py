"""Catálogo de productos, categorías y depósitos de Contalibra, sobre las
tablas de LibraCommerce (`catalog_items`, `item_codes`, `categories`,
`locations`) — P7 del plan de consolidación de la familia Libra (ver
...
módulos existían antes que LibraCommerce). `libracore.db.productos` sigue
existiendo sin cambios para **Restolibra**, que no se migra (su stock es
receta-aware, dominio gastronómico sin equivalente en LibraCommerce).
...
frontend (`frontend/src/api.ts`, tipos `Producto`/`Deposito`) dependen de
los nombres de columna de Contalibra, no de los de LibraCommerce. El mapeo
vive acá y en ningún otro lado.

Las tablas de LibraCommerce viven en el MISMO archivo SQLite que el resto
de Contalibra (decisión del plan: `crear_venta_directa`/`anular_venta` son
una única transacción atómica que cruza ambos motores, y separarlos en dos
archivos rompería esa atomicidad para datos financieros reales).
"""
import json
import re
...
def get_stock_por_deposito(deposito_id: int) -> list[dict]:
    with get_connection() as conn:
        rows = conn.execute("""
            ...
            -- Ver el comentario de db_stock.get_stock_todos: las columnas
            -- de tablas unidas tienen que estar en el GROUP BY.
            GROUP BY ci.id, cat.name, ic.code
            -- Y la expresion se repite en vez de usar el alias: PostgreSQL no
            -- acepta alias de la SELECT en el HAVING (SQLite si).
...
def _resolver_categoria_id(conn, categoria: str) -> int | None:
    """Contalibra guarda la categoría como texto libre en el producto;
