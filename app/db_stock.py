"""Movimientos de stock de Contalibra, sobre `stock_movements` de
LibraCommerce — P7 del plan de consolidación de la familia Libra (ver
...
`db_productos.py`: el inventario es dominio de LibraCommerce. `libracore.db.stock`
sigue existiendo sin cambios para **Restolibra**, que no se migra — su
`descontar_stock_venta` es receta-aware vía el hook `configure_resolver_receta`,
un dominio gastronómico sin equivalente en LibraCommerce. Contalibra nunca
configuró ese hook, así que acá va la versión simple (descuenta el propio
producto), que es exactamente el comportamiento que Contalibra ya tenía.

**Mapeo de `tipo`**: el vocabulario de Contalibra es más fino que el enum
del motor — `entrada`/`salida`/`ajuste` son los tres `ADJUSTMENT` para
LibraCommerce, pero la UI los muestra con iconos y semántica distintos. El
tipo original se preserva en `stock_movements.reason_code` (agregado en la
migración 0003 de LibraCommerce justo para esto) y es el que se devuelve al
leer; `movement_type` queda como el tipo semántico del motor.
...
# los de Contalibra, que siempre lo escriben.
...
def add_movimiento_stock(producto_id: int, tipo: str, cantidad: float,
    ...
    # Contalibra maneja `fecha` como 'YYYY-MM-DD'; `occurred_at` es un
...
def get_stock_todos() -> list[dict]:
    ...
    with get_connection() as conn:
        rows = conn.execute("""
            ...
            -- tablas: PostgreSQL solo deja seleccionar fuera del grupo las
            -- columnas de la tabla cuya clave primaria se agrupa. SQLite lo
            -- aceptaba y elegia una fila cualquiera.
...
def descontar_stock_venta(venta_id: int, items: list, fecha: str = "",
    ...
    A diferencia de `libracore.db.stock.descontar_stock_venta`, acá no hay
    resolver de recetas ni modificadores: eso es dominio de Restolibra, y
    Contalibra nunca configuró el hook. El comportamiento resultante es
    idéntico al que Contalibra ya tenía (descuenta el propio producto).

    Si se pasa `conn`, corre dentro de esa transacción (ej. `crear_venta_directa`):
