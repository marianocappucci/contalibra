"""Listas de precio de Contalibra, sobre `price_lists`/`item_prices` de
LibraCommerce — P7b (ver
...
El modelo de Contalibra es "flat" — un precio por producto por lista, sin
...
usar la fecha real (Contalibra nunca tuvo ese dato).

`libracore.db.listas_precio` sigue existiendo sin cambios para
**Restolibra**, que no se migra.
"""
...
# con que el modelo de Contalibra nunca tuvo noción de vigencia. Mismo
...
def _upsert_precio(conn, lista_id: int, producto_id: int, precio: float) -> None:
    ...
    (vigencias/quiebres/sucursal). Contalibra solo escribe la fila "sin
