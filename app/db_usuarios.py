...
(login, credenciales, el dashboard de actividad, el borrado del usuario de
demo) y siguen traduciendo la forma del dict a la de siempre --
`get_usuario_by_username`/`check_usuario_credentials` alimentan
`app/web/auth.py` (`SessionAuth`) y `app/web/api_auth.py`, que **no** hablan
el contrato de libraauth.
...
esperan `web/auth.py`, `web/api_auth.py` y el resto del dominio.

**La tabla `usuarios` no se movio**: vive en `contalibra.db`, que es tambien la
base de LibraCore, y el engine apunta ahi. Es a proposito — 12 tablas de este
producto declaran `usuario_id REFERENCES usuarios(id)` y esas FK resuelven
...
# rechazaria `operador` y `cajero`. `staff` no existe en este producto, y el
...
# `contalibra.db` junto al resto del dominio.
#
...
# libraauth. La tabla `secretos_instancia` la crea la revision `0002` de la
# cadena de libraauth —no un `create_all`, que este modulo dejo de correr en la
# v0.45.0—, y `exigir_schema_al_dia()` del arranque no deja levantar la app si
# esa revision no se aplico.
#
...
# ~12 consumidores, pero lo trae de la base cifrada y no del archivo. La
...
_password_reset = PasswordResetService(
    ...
    reset_url_base=os.environ.get(
        "CONTALIBRA_RESET_URL_BASE", "https://dev.contalibra.com.ar/reset-password"
    ),
