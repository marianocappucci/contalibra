"""Baseline: las tablas propias de Contalibra tal como ya existen.

Esta revisión **llama a `init_schema_propio()`** en vez de re-expresar sus 3
...
Desde esta revisión, `init_schema_propio()` es de **sólo lectura**: todo cambio
de schema va como revisión nueva. Lo sostiene
`tests/test_schema_propio_congelado.py`, no la memoria de quien edite.

**Se puede correr sobre una instancia viva**, y es lo que se hace: la función es
idempotente, así que `alembic upgrade head` sobre una base que ya tiene el
schema hace lo mismo que un arranque de la app, más registrar la versión. Por
eso las 3 instancias existentes **se migran, no se estampan**: el resultado es
el mismo y además queda verificado.

🔴 **Por qué llama a `init_commerce_schema()` primero.** Dos de las tres tablas
—`venta_links` y `ventas_origen_externo`— tienen FK contra `sales`, que es de
LibraCommerce. Y LibraCommerce **no tiene cadena de Alembic**: sus tablas
nacen de `init_schema()` al conectar, o sea *después* de que corren las
migraciones. En una instancia viva eso no se nota (las tablas están hace meses),
pero en un alta nueva las migraciones corren **antes del primer arranque** —ver
...
revision = "0001_baseline_contalibra"
down_revision = None
branch_labels = None
depends_on = None
...
def downgrade():
    # Bajar de la baseline es borrar las tablas que atan cada venta con su
    # factura, su remito y su turno de caja. No hay caso de uso que lo
    # justifique y sí una forma muy barata de perder la trazabilidad de una
    # instancia: el rollback de esta revisión es restaurar el backup.
