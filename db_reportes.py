"""Reportes agregados de solo lectura de Contalibra.

El dominio sigue siendo de LibraCore, pero **cinco de sus siete funciones
leen `ventas` o `productos`**, que en Contalibra ya no son la fuente de
verdad desde P7 (ver `db_ventas.py` / `db_productos.py`). Se redefinen acá
...
LibraCore) se re-exportan tal cual. **LibraCore no se toca** — Restolibra
