...
if not SECRET_KEY:
    if os.environ.get("ENV", "production") == "development":
        SECRET_KEY = "contalibra-secret-change-me"
    else:
...
def require_role(*roles: str):
    ...
    roles indicados. Roles del sistema: operador | admin.

    Antes de esto, `require_admin` era la única forma de restringir por rol y
    solo se usaba en `usuarios.py`/`logs.py` (y `anular_venta`) — el resto
    (config, tesorería, facturación, cuenta corriente, libros IVA) sólo exigía
    estar logueado, dejando a `operador` con acceso idéntico a `admin` (ver
    wiki/analyses/restolibra-auditoria-produccion, hallazgo cruzado a este
    repo)."""
