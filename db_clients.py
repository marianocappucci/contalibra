...
asociadas. Extraído de database.py como parte del split en módulos
...
`get_client_by_email`/`get_client_by_cuit` vivían mal ubicadas bajo el
header "MercadoPago pagos" en el archivo original (usadas ahí para
resolver el payer de un pago MP a un cliente) — son funciones de dominio
"clientes" por tabla que consultan, así que se movieron acá con el resto.
"""
import json
