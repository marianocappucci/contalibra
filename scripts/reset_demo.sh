#!/usr/bin/env bash
# Reset diario de la demo publica de Contalibra.
#
...
# 2026-08-10 habia una copia suelta en `/root/scripts-demo/reset_contalibra.sh`
...
SEED_LOCAL=/tmp/seed-contalibra.py
git -C /root/contalibra fetch -q origin || { log "ABORTA: no se pudo hacer fetch de contalibra."; exit 5; }
git -C /root/contalibra show origin/develop:scripts/seed_demo.py > "$SEED_LOCAL" || { log "ABORTA: no esta scripts/seed_demo.py en origin/develop."; exit 6; }
...
URL_BASE=$(docker exec "$CONTENEDOR" printenv CONTALIBRA_DATABASE_URL 2>/dev/null || true)
...
import sqlite3
try:
    c = sqlite3.connect('/app/data/contalibra.db')
