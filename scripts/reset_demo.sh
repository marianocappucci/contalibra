#!/usr/bin/env bash
# Reset diario de la demo publica de Contalibra — item 8 de los pendientes de
# Libra.
#
...
docker cp "/root/contalibra/scripts/seed_demo.py" "$CONTENEDOR:/tmp/seed.py"
docker exec -i "$CONTENEDOR" sh -c '
  python3 /tmp/seed.py \
    --url https://demo.contalibra.com.ar \
    --usuario "${CONTALIBRA_ADMIN_USERNAME:-admin}" \
    --password "$CONTALIBRA_ADMIN_PASSWORD"
'
