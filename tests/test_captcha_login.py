...
1. Que `GET /api/captcha` exista y devuelva un desafio con la forma que espera
...
2. 🔴 Que un login SIN captcha rebote con 400, aunque la contrasena sea buena.
   Si alguien sacara `captcha=True` de `app/web/api/auth.py`, esto es lo que
   se pone rojo.
...
from tests.conftest import _CAPTCHA_DE_ORIGINAL, ADMIN_PASS, ADMIN_USER


@pytest.fixture(autouse=True)
def _captcha_real(monkeypatch):
    ...
    segundo que cuesta en el navegador. El `raising=False` es porque la app de
    modulo puede no tenerlo todavia; el `monkeypatch` lo saca al terminar, asi
    no le queda a los tests que vienen despues.
    """
    monkeypatch.setattr("libraauth.session_auth._captcha_de", _CAPTCHA_DE_ORIGINAL)
    monkeypatch.setattr(
        app.state,
        "captcha",
        Captcha("clave-de-prueba", costo=1, contador_min=1, contador_rango=5),
        raising=False,
    )
...
def test_login_con_el_desafio_resuelto_entra(client):
    ...
    desafio = Challenge.from_dict(client.get("/api/captcha").json())
