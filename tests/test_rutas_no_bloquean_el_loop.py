...
Cada test se probó volviendo su ruta a como estaba: los cuatro se ponen rojos.
"""
import asyncio
import threading
import time
...
from app import arca_wsaa, arca_wspadron, venta_facturacion
...
# ── POST /api/integraciones/consultas ──────────────────────────────────────

def test_registrar_consulta_no_frena_el_loop(admin_client, monkeypatch):
    """La venta sola, sin factura: lo sincrónico es la base."""
    puesto = admin_client.put("/api/integraciones/config", json={"username": "admin"})
    assert puesto.status_code == 200, puesto.text
    lento = _Lento()
    real = db.crear_venta_directa

    def _venta_lenta(**kw):
        lento.dormir()
        return real(**kw)

    monkeypatch.setattr(db, "crear_venta_directa", _venta_lenta)

    respuesta, health, fin = _mientras_duerme(
        lento, lambda c: c.post("/api/integraciones/consultas", json=_consulta()),
        cookies=admin_client.cookies)

    assert respuesta.status_code == 200, respuesta.text
    assert respuesta.json()["ya_existia"] is False
    _no_bloqueo(lento, health, fin)


def test_facturar_la_consulta_no_frena_el_loop(admin_client, monkeypatch):
    """🔑 Con factura, lo lento va ADENTRO de `facturar_venta` —el numerador,
    que en producción autentica contra el WSAA firmando con `openssl`—. Es el
    caso que el `await` desde el loop no resolvía aunque la ruta fuera `def`."""
    puesto = admin_client.put("/api/integraciones/config", json={"username": "admin"})
    assert puesto.status_code == 200, puesto.text
    lento = _Lento()
    real = venta_facturacion.get_next_numero_with_arca

    async def _numerar_con_openssl(punto_venta, tipo):
        lento.dormir()
        return await real(punto_venta, tipo)

    monkeypatch.setattr(venta_facturacion, "get_next_numero_with_arca", _numerar_con_openssl)

    respuesta, health, fin = _mientras_duerme(
        lento,
        lambda c: c.post("/api/integraciones/consultas", json=_consulta(facturar=True)),
        cookies=admin_client.cookies)

    assert respuesta.status_code == 200, respuesta.text
    assert respuesta.json()["factura"] is not None, "la consulta tenía que facturarse"
    _no_bloqueo(lento, health, fin)
