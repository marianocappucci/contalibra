...
Este archivo nace el 2026-08-25, con la cadena propia de Contalibra. Sostiene
...
1. **Que las dos puntas no diverjan.** El arranque (`init_db()`) y la baseline
   (`migrations/versions/0001_baseline_contalibra.py`) llaman a la misma
...
def test_la_secuencia_declarada_levanta_el_schema_desde_cero():
    ...
    🔴 El orden no es decorativo, y está medido: corriendo **sólo** la cadena
    propia contra una base vacía, la baseline muere con
    `relation "facturas" does not exist` — `venta_links` referencia `facturas`,
    `remitos` y `turnos_caja`, que son de LibraCore. Por eso `libracore-migrar`
    va primero en la declaración, y por eso este test corre la secuencia entera
    y no el último comando solo.
    """
    _vaciar()
    _correr_la_cadena()
...
def test_las_dos_cadenas_no_comparten_la_tabla_de_version():
    """`alembic_version_contalibra` para la propia, `alembic_version` para el motor.

    🔴 Compartir el nombre haría que cada cadena leyera la revisión de la otra:
    la del motor encontraría `0001_baseline_contalibra`, no la reconocería, y el
