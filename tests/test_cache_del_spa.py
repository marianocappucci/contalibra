...
def cliente(tmp_path_factory):
    ...
    if creado:
        (dist / "index.html").write_text(
            "<!doctype html><title>Contalibra</title>", encoding="utf-8"
        )
    ...
    previo = {v: os.environ.get(v) for v in
              ("ENV", "CONTALIBRA_ADMIN_USERNAME", "CONTALIBRA_ADMIN_PASSWORD",
               "SECRET_KEY", "DATA_DIR")}
    os.environ["ENV"] = "development"
    os.environ["CONTALIBRA_ADMIN_USERNAME"] = "admin"
    os.environ["CONTALIBRA_ADMIN_PASSWORD"] = "clave-de-prueba"
    ...
    modulo = importlib.import_module("app.web.app")
...
def test_una_ruta_del_ruteo_del_cliente_tambien(cliente):
    ...
    r = cliente.get("/una-ruta-del-cliente")
    assert r.status_code == 200
    assert "no-cache" in r.headers.get("cache-control", ""), dict(r.headers)
