...
def _schema_de(conn, tabla):
    ...
    que es lo unico que estos tests necesitan: una cadena donde
    `REFERENCES sales(` aparezca si la FK apunta ahi.
    """
...
def test_migracion_repunta_la_fk_vieja_conservando_las_filas(client):
    ...
    with db_core.get_connection() as conn:
        if not db_core.ES_POSTGRES:
            ...
            # proposito: apagar las FK es una promesa que alla no se puede
            # cumplir. Tampoco hace falta para rehacer esta tabla.
        ...
        # `ventas`, PostgreSQL exige que exista ahi para aceptar el INSERT de
        # abajo. En SQLite da igual porque el pragma esta apagado, asi que
        # sembrar las dos reproduce el mismo estado en los dos motores.
        conn.execute("INSERT INTO ventas (id, numero, fecha, items, total) VALUES (7, 'V-7', '2026-08-10', '[]', 100)")
    ...
    with db_core.get_connection() as conn:
        ...
        # Y la FK quedo realmente operativa, no solo declarada. En
        # PostgreSQL no hay equivalente porque no hace falta: la integridad se
        # aplica siempre, no existe el estado "declarada pero no verificada"
        # que SQLite permite con el pragma apagado.
        if not db_core.ES_POSTGRES:
...
def test_migracion_conserva_las_filas_sin_venta_y_avisa(client, capfd):
    """El estado real de contalibra-dev el 2026-07-31: entorno a medio
    migrar de P7 (datos en la tabla vieja `ventas`, `sales` vacia). Esas
    ...
    with db_core.get_connection() as conn:
        ...
        """)
        # venta_id 99 existe en la vieja `ventas` y NO en `sales`: es
        # exactamente el estado "a medio migrar de P7". Sembrarlo en `ventas`
        # es lo que hace que PostgreSQL acepte el INSERT con la FK vieja.
        conn.execute("INSERT INTO ventas (id, numero, fecha, items, total) VALUES (99, 'V-99', '2026-08-10', '[]', 500)")
        conn.execute(
...
def test_migracion_es_idempotente(client):
    ...
    with db_core.get_connection() as conn:
        ...
        # Solo aplica a SQLite: en PostgreSQL la migracion cambia la constraint
        # con dos ALTER y nunca renombra la tabla, asi que no hay `_old` posible.
        if not db_core.ES_POSTGRES:
            viejas = conn.execute(
...
def test_backfill_de_parties_repara_una_base_a_medio_migrar(client, admin_client):
    """El caso de la base del cliente real: clientes creados despues de P7
