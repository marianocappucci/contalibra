 ...
 *  elemento tapado (la primera corrida del CI murió esperando que la casilla
 *  quedara "visible, enabled and stable"). El label está asociado a la casilla,
 *  que es de donde sale su nombre accesible: es lo mismo que hace el humano.
 *  Y `click()` y no `check()`: la casilla queda tildada recién cuando termina
 *  la prueba de trabajo (~1 s), y `check()` exige que el estado cambie en el
 *  acto. Lo que se espera es lo que ve el humano: el botón habilitado.
 */
...
test('entra por /login, llega al Dashboard y abre los comprobantes', async ({ page }) => {
