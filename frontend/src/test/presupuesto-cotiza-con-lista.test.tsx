// El presupuesto cotiza con la lista del cliente/elegida (slice 3) y re-cotiza el
// renglón por cantidad cuando hay quiebres (slice 4, add-on mayorista).
//
// El mecanismo ya existía en el backend (`/productos/buscar?lista_id=...` y
// `/api/listas-precio/{id}/precio?...&cantidad=`); acá se cuida que el formulario
// los use. Un presupuesto que ignora la lista o el quiebre no tira ningún error:
// da un número, y da el equivocado.
//
// Se usa el modo EDICIÓN a propósito: el cliente y su lista quedan preseleccionados
// sin operar los <Select>.
...
function montarEdicion({ listaAsignada, listas, modulos = ['clientes', 'listas_precio', 'mayorista'] }:
  { listaAsignada: number | null; listas: unknown[]; modulos?: string[] }) {
  fetchMock = vi.fn((url: string) => {
    ...
    if (u.includes('/api/listas-precio/5/precio')) {
      // El quiebre: 10+ baja de 80 a 70. Delata que se re-cotizó por cantidad.
      const cant = Number(new URL(u, 'http://x').searchParams.get('cantidad'))
      return Promise.resolve(json({ precio: cant >= 10 ? 70 : 80 }))
    }
    if (u.includes('/api/listas-precio')) return Promise.resolve(json(listas))
    if (u.includes('/productos/buscar')) {
      // Como el backend: precio de lista (80) sólo si se pasó lista_id; sin él, base (100).
...
async function elegirFideos(usuario: ReturnType<typeof userEvent.setup>) {
...
describe('el presupuesto cotiza con la lista (add-on mayorista, slices 3 y 4)', () => {
  it('preselecciona la lista del cliente y cotiza el producto a esa lista', async () => {
    ...
    expect(numericos[1].value).toBe('80')  // precio de lista, no el base (100)
  })
  ...
  it('re-cotiza el renglón por cantidad cuando cambia la cantidad (quiebre)', async () => {
    ...
    const numericos = () => screen.getAllByRole('spinbutton') as HTMLInputElement[]
    await usuario.clear(numericos()[0])
    await usuario.type(numericos()[0], '10')

    const llamadas = fetchMock.mock.calls.map((c) => String(c[0]))
    expect(llamadas.some((u) => u.includes('/api/listas-precio/5/precio') && u.includes('cantidad=10'))).toBe(true)
    await vi.waitFor(() => expect(numericos()[1].value).toBe('70'))  // precio del quiebre
  })

  it('sin listas (modulo listas_precio apagado) no muestra el selector', async () => {
    montarEdicion({ listaAsignada: null, listas: [], modulos: ['clientes'] })
