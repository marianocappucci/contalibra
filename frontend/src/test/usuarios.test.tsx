...
// los roles (admin/operador/cajero, sin "mozo" -- eso es Restolibra),
// `permitirEliminar` y `usuarioActualId`. Es exactamente lo que un cambio
// bien intencionado en el shim podría romper sin que se note en el resto
// de la suite (que no monta esta pantalla).
...
const OPERADOR = {
  id: '2', username: 'op1', name: 'Op Uno', role: 'operador', active: true, email: '',
}
...
beforeEach(() => {
  fetchMock = vi.fn((url: string) => {
    ...
    if (u.includes('/api/usuarios')) return Promise.resolve(json([ADMIN, OPERADOR]))
    return Promise.resolve(json([]))
  })
...
describe('Usuarios', () => {
  it('lista los usuarios pegándole a /api/usuarios (basePath propio)', async () => {
    montar()
    expect(await screen.findByText('Op Uno')).toBeInTheDocument()
  ...
  // ofrecería un chequeo más directo de las tres opciones): el trigger
  // consulta `hasPointerCapture`, que jsdom no implementa, y el menú nunca
  // llega a montarse -- mismo límite ya documentado en
  // `pos-monto-sugerido.test.tsx`. Lo que sí se puede probar sin abrirlo es
  // que un rol PROPIO (`operador`, que no es el default de libra-ui) llega
  // intacto de ida y de vuelta.
  it('un rol propio (operador) se edita y se guarda tal cual, sin tocar el select', async () => {
    montar()
    await screen.findByText('Op Uno')
    const usuario = userEvent.setup()

    await usuario.click(screen.getByRole('button', { name: 'Editar Op Uno' }))
    ...
    // Si `roles` no le hubiera llegado al componente (o le hubiera llegado
    // el default de libra-ui, `staff`), esto no sería 'operador'.
    expect(cuerpo.role).toBe('operador')
  })
  ...
  it('el botón Eliminar no aparece en la fila del propio usuario logueado', async () => {
    ...
    // Eliminar -- borrarse a uno mismo ya lo rechaza el backend, pero no hay
    // por qué ofrecer el botón para eso.
    ...
    expect(await screen.findByRole('button', { name: 'Eliminar Op Uno' })).toBeInTheDocument()
  })
