...
type NavItem = { to: string; label: string; icon: React.ComponentType<{ className?: string }>; module?: string; adminOnly?: boolean; children?: NavChild[] }
    ...
    items: [
      {
        ...
        children: [{ to: '/listas-precio', label: 'Listas de precios', module: 'listas_precio' }],
      },
...
// Iconos usados en items anidados sin icono propio (nested items no llevan
// icono en el sidebar viejo salvo estos dos casos puntuales).
    ...
    <Sidebar collapsible="icon">
      <SidebarHeader>
        <NavLink to="/dashboard" className="flex items-center gap-2 px-2 py-1.5">
          ...
          <div className="flex min-w-0 flex-col group-data-[collapsible=icon]:hidden">
            <span className="truncate font-semibold">Contalibra</span>
            {user?.empresa_nombre && (
      ...
      <SidebarContent>
        {NAV_SECTIONS.map((section, si) => {
          const items = section.items.filter((item) => (!item.adminOnly || isAdmin) && visible(item))
