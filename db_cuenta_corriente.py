...
`facturas` — nada de eso se migra en P7). Pero **tres de sus funciones hacen
`JOIN ventas`** para computar los débitos por ventas a cuenta corriente, y en
Contalibra las ventas viven en `sales` desde P7 (ver `db_ventas.py`).

Este módulo re-exporta todo lo de LibraCore salvo las funciones afectadas,
que se redefinen acá contra `sales`. **LibraCore no se toca** — Restolibra
sigue funcionando igual.

Se redefine también `get_cc_movimientos_periodo` (el resumen que se manda por
mail) aunque no toque `ventas` directamente: se calcula sobre
`get_cc_movimientos`, y si usara el de LibraCore leería la tabla vieja — el
resumen enviado al cliente **no cerraría con el saldo de la pantalla**.
...
def get_cc_movimientos_periodo(cliente_id: int, desde: str, hasta: str) -> dict:
    ...
    `get_cc_movimientos()` — el de este módulo — en vez de repetir las tres
    consultas: la cuenta corriente es chica por cliente y así no hay riesgo de
    que el resumen que se manda por mail se calcule distinto que la pantalla.
    """
